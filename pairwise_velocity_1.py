...
def make_csv(data_name, deg, num, particle):  # string, float (rotation angle in degrees)
    ...
    idlowmass = np.unique(data['satid'])[data2['msat'][1211:(1515 - 149)] > 0.5e10]
...
testd = make_csv('halo17', 0, 60000,False)
...
df1['satid'] = testd['satid'][ (zmin < r2 ) & (r2 < zmax ) ]
#print (df1['index'])
...
df2['satid'] = df1['satid'][(np.absolute(df1['z']) > zlim)]

df3 = df2.sort_values(by=['index'])

df3.to_csv("df3_60000_nolowmass.csv")
...
np.savetxt('halo17_scale2_numf_60000.txt', numf, delimiter=' ')
np.savetxt('halo17_scale2_indexf_60000.txt', indexf[2], delimiter=' ',fmt = 'str')
...
for sc in range(0, len(test1f)):
    for k in range(0, len(test1f[1])):
        ...
        dff[str(sc)+'-'+str(k)+' '+str(int(rlimf[sc][k])) + '-' + str(int(rlimf[sc][k] + 8)) + 'kpc y'] = hist
        dff[str(sc)+'-'+str(k)+' '+str((int(rlimf[sc][k]))) + '-' + str(int(rlimf[sc][k] + 8)) + 'kpc x'] = bin_edges[:100]

dff.to_csv("halo17_veldiff_60000.csv")
